defmodule Resdayn.Catalog.Export.OverrideTest do
  use Resdayn.IntegrationCase

  alias Resdayn.Catalog.Export.Override
  ...
  describe "exportable extension" do
    test "updating a tool automatically creates an override" do
      tool = Ash.get!(Resdayn.Catalog.Items.Tool, "hammer_repair")
    ...
    test "updating a tool twice upserts the same override" do
      tool = Ash.get!(Resdayn.Catalog.Items.Tool, "pick_master")
    ...
    test "updating a tool upserts its search index entry" do
      tool = Ash.get!(Resdayn.Catalog.Items.Tool, "probe_grandmaster")

      tool
      |> Ash.Changeset.for_update(:update, %{name: "Renamed Probe"})
      |> Ash.update!()

      entry = Ash.get!(Resdayn.Catalog.Search.SearchIndex, "tool:probe_grandmaster")
  ...
  describe "record calculation" do
    test "loads the referenced record for an override" do
      ...
      assert loaded.record.__struct__ == Resdayn.Catalog.Items.Tool
    ...
    test "loads records for multiple overrides of different types" do
      ...
      assert loaded1.record.__struct__ == Resdayn.Catalog.Items.Tool
      assert loaded2.record.__struct__ == Resdayn.Catalog.Items.Tool
