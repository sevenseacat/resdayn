defmodule Resdayn.Catalog.World.InventoryItem do
  use Ash.Resource,
    otp_app: :resdayn,
    domain: Resdayn.Catalog.World,
    data_layer: AshPostgres.DataLayer,
    extensions: [Resdayn.Catalog.Importable]

  postgres do
    table "inventory_items"
    repo Resdayn.Repo

    references do
      reference :holder_ref, index?: true, on_delete: :delete
      reference :object_ref, index?: true, on_delete: :delete
    end
  end

  actions do
    defaults [:read, :create, :update, :destroy]
    default_accept [:count, :restocking?, :holder_ref_id, :object_ref_id]
  end

  attributes do
    attribute :count, :integer, allow_nil?: false, constraints: [min: 1], default: 1
    attribute :restocking?, :boolean, allow_nil?: false, default: false
  end

  relationships do
    belongs_to :holder_ref, Resdayn.Catalog.World.ReferencableObject,
      allow_nil?: false,
      primary_key?: true

    belongs_to :object_ref, Resdayn.Catalog.World.ReferencableObject,
      allow_nil?: false,
      primary_key?: true
  end

  calculations do
    calculate :holder, :struct, {Resdayn.Catalog.Calculations.TypedObject, field: :holder_ref}
    calculate :object, :struct, {Resdayn.Catalog.Calculations.TypedObject, field: :object_ref}
  end
end
